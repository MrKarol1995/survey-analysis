...
# Parametry symulacji
alpha = 0.05  # Poziom istotności (1 - alfa = 0.95)
n_values = [30, 100, 500]  # Zmniejszony rozmiar prób dla optymalizacji
p_values = np.linspace(0.1, 0.9, 5)  # Mniej wartości prawdopodobieństwa
num_simulations = 2000  # Zmniejszona liczba powtórzeń symulacji

# Funkcja do obliczania przedziałów ufności
methods = ['binom_test', 'normal', 'wilson']  # Clopper-Pearson, Wald, Wilson

def simulate_coverage_and_length(n, p, method):
    x_samples = np.random.binomial(n, p, num_simulations)
    intervals = np.array([proportion_confint(x, n, alpha=alpha, method=method) for x in x_samples])
    coverages = np.mean((intervals[:, 0] <= p) & (intervals[:, 1] >= p))
    lengths = np.mean(intervals[:, 1] - intervals[:, 0])
    return coverages, lengths

def run_simulation(method, n, p):
    return p, *simulate_coverage_and_length(n, p, method)

# Przechowywanie wyników
results = {method: {n: {'coverage': [], 'length': []} for n in n_values} for method in methods}

# Przeprowadzanie symulacji z równoległym przetwarzaniem (ograniczenie procesów)
for method in methods:
    for n in n_values:
        output = Parallel(n_jobs=2)(delayed(run_simulation)(method, n, p) for p in p_values)
        for p, coverage, length in output:
            results[method][n]['coverage'].append(coverage)
            results[method][n]['length'].append(length)
...
for j, n in enumerate(n_values):
    for i, metric in enumerate(['coverage', 'length']):
        ax = axes[i, j]
        for method in methods:
            ax.plot(p_values, results[method][n][metric], label=method)
        if metric == 'coverage':
            ax.axhline(0.95, linestyle='dashed', color='gray')
        ax.set_title(f"n = {n}, {metric}")
        ax.set_xlabel("p")
